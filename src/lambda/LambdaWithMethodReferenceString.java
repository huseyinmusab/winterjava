...
public class LambdaWithMethodReferenceString {
	...
	*/
	public static void printStartsWithA(List<String> l) {
		...
		//"startWithA()" method is very specific method. Reuseability is not possible. For that kind of situations
		//no need to use "method reference" use "lambda expression"
		l.stream().filter(Utils::startsWithA).map(String::toUpperCase).forEach(System.out::println);	
	}
